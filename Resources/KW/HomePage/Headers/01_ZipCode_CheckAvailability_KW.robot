...
Enter Zip Code and select Check Availability Button
    ${Zip_Code1_Inp}  read excel data by cell name  ${EXCEL_LOC}  ${SHEET_NAME}  ${TESTCASE_NO}  ${ZIP_CODE_NUM1}
...
Verify header now changes to "Plano,TX"
    ${Expected_Location_Text}  read excel data by cell name  ${EXCEL_LOC}  ${SHEET_NAME}  ${TESTCASE_NO}  ${ZIP_CODE1_TEXT}
...
Select Change link again and enter zip code and select Check Availability Button
    ${Zip_Code2_Inp}  read excel data by cell name  ${EXCEL_LOC}  ${SHEET_NAME}  ${TESTCASE_NO}  ${ZIP_CODE_NUM2}
...
Verify black header now changes to 'Fairport NY'
    ${Expected_Location_Text}  read excel data by cell name  ${EXCEL_LOC}  ${SHEET_NAME}  ${TESTCASE_NO}  ${ZIP_CODE2_TEXT}
